...
class SharedSchemaDialect extends SchemaDialect {
    ...
    getAnnotations(fragment) {
        return mobx.observable.object(getAnnotations(fragment));
    }
    getValidations(fragment, types) {
        return mobx.observable(getValidations(fragment, types));
    }
}
...
class JSONSchemaDraft7Dialect extends SharedSchemaDialect {
    getCombiners(fragment) {
        return mobx.observable.set(getCombiners(fragment));
    }
    getAnnotations(fragment) {
...
function getCombiners$1(fragment) {
    ...
    return mobx.observable.set(combiners);
}
...
function getValidations$2(schema, types) {
    return mobx.observable.object(getMatchingValidations(schema, VALIDATION_TYPES$2, types));
}

class OAS2SchemaObjectDialect extends SharedSchemaDialect {
    getCombiners(fragment) {
        return mobx.observable.set(getCombiners$1(fragment));
    }
    getAnnotations(fragment) {
        return mobx.observable.object({
            ...super.getAnnotations(fragment),
...
class BaseNode {
    constructor(fragment) {
        ...
        mobx.makeObservable(this, {
            subpath: mobx.observable.shallow,
        });
    }
    get path() {
...
class MirroredRegularNode extends BaseNode {
    ...
    get children() {
        ...
        if (!this._reaction) {
            ...
            this._reaction = mobx.reaction(() => referencedChildren, mobx.action(() => {
                console.log('foo!');
            }));
        }
...
function getPrimaryType(fragment, types) {
    if (types.size > 0) {
        if (types.has(exports.SchemaNodeKind.Object)) {
            return exports.SchemaNodeKind.Object;
        }
        if (types.has(exports.SchemaNodeKind.Array)) {
...
function getTypes(fragment) {
    if ('type' in fragment) {
        if (Array.isArray(fragment.type)) {
            return mobx.observable.set(fragment.type.filter(isValidType));
        }
        else if (isValidType(fragment.type)) {
            return mobx.observable.set([fragment.type]);
        }
    }
    const inferredType = inferType(fragment);
    if (inferredType !== null) {
        return mobx.observable.set([inferredType]);
    }
    return mobx.observable.set([]);
}
...
class RegularNode extends BaseNode {
    constructor(fragment, dialect) {
        ...
        this.enum = mobx.observable.set('const' in fragment ? [fragment.const] : unwrapArrayOrNull(fragment.enum));
    ...
    walk() {
        ...
        if ('children' in initialSchemaNode && !isRootNode(schemaNode)) {
            if (initialSchemaNode.children === void 0) {
                ...
                mobx.makeObservable(initialSchemaNode, {
